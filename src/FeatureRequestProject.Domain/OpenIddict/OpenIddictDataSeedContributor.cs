...
namespace FeatureRequestProject.OpenIddict;
...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("FeatureRequestProject") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "FeatureRequestProject", DisplayName = "FeatureRequestProject API", Resources = { "FeatureRequestProject" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["FeatureRequestProject_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["FeatureRequestProject_Web:RootUrl"]!.EnsureEndsWith('/');

            /* FeatureRequestProject_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["FeatureRequestProject_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["FeatureRequestProject_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["FeatureRequestProject_Swagger:RootUrl"]?.TrimEnd('/');
